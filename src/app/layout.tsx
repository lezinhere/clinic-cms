...
const NAV_ITEMS = {
  ...
  DOCTOR: [
    ...
    { label: "Consultation", href: "/doctor/consult" },
  ],
  PHARMACY: [
    { label: "Prescriptions", href: "/pharmacy/queue" },
  ],
  LAB: [
...
function Providers({ children }: { children: React.ReactNode }) {
  const { AuthProvider } = require("@/lib/auth") // Dynamic import to avoid SSR issues with local storage if needed, but standard import is fine here given client directive.
  // Actually, import at top is better.
...
function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // Routes that should NOT have a sidebar for guests (Public/Kiosk pages)
  const FORCED_PUBLIC_ROUTES = ["/", "/care-connect", "/care-connect/login"];
  const isForcedPublic = FORCED_PUBLIC_ROUTES.includes(pathname);

  // If No User OR it's a forced public page, hide sidebar
  if (!user || isForcedPublic) {
    return <main className="min-h-screen bg-transparent">{children}</main>
  }

  // If on patient routes without being a patient, hide sidebar (security precaution)
